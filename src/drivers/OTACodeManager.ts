 ...
 * @returns {string} 
 */
export function generate(payload: string | Object | Buffer, action: ACCOUNT_ACTIONS, expiresIn?: string): Promise<OTACode> {
    ...
    return new Promise<OTACode>((resolve, reject) => {
        jwt.sign(
            ...
            (err, token) => {
                token = token.replace(process.env.TOKEN_REPLACER, process.env.OTA_CODE_REPLACEMENT);
                err ? reject(err)
                    : resolve(
                        { id: otaID, code: token }
                    )
 ...
 * @returns {Promise<any>} 
 */
export function decode(otaCode: string, otaID: string): Promise<DecodedOTACode> {
    otaCode = otaCode.replace(process.env.OTA_CODE_REPLACER, process.env.TOKEN_REPLACMENT);
